...
func (h *Handlers) HandleGetTimeRange(c *fiber.Ctx) error {
	var queryParams db.GetTimeRangeParams
	if err := c.QueryParser(&queryParams); err != nil {
		return c.Status(http.StatusBadRequest).JSON(err.Error())
	}

	entries, err := h.Store.GetTimeRange(c.Context(), queryParams)
	if err != nil {
		return c.Status(http.StatusInternalServerError).JSON(err.Error())
	}
...
func (h *Handlers) HandleGetTimeRangeByUserID(c *fiber.Ctx) error {
	...
	var queryParams db.GetTimeRangeByUserIDParams
	if err := c.QueryParser(&queryParams); err != nil {
		return c.Status(http.StatusBadRequest).JSON(err.Error())
	}
	...
	entries, err := h.Store.GetTimeRangeByUserID(c.Context(), queryParams)
	if err != nil {
		return c.Status(http.StatusInternalServerError).JSON(err.Error())
	}
